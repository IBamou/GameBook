        ...
        <div class="flex justify-between items-start mb-8">
            ...
            @auth
                ...
                <div class="flex gap-2">
                    ...
                    <form action="{{ route('categories.delete', $category) }}" method="POST" onsubmit="return confirm('Delete this category?')" class="inline">
                        @csrf
                        @method('DELETE')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($category->games as $game)
                    <a href="{{ route('games.show', $game) }}" class="card-hover group overflow-hidden">
                        <div class="aspect-video bg-gradient-to-br from-sky-100 to-slate-100 flex items-center justify-center">
                            @if($game->image_url)
                        ...
                        <div class="p-4">
                            <h3 class="font-semibold text-slate-950 group-hover:text-sky-600 transition-colors">{{ $game->name }}</h3>
                            <p class="text-xs text-slate-500 mt-1">{{ $game->difficulty }} • {{ $game->min_players }}–{{ $game->max_players }} players</p>
